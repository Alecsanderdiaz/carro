...
#
				print("????????????????????????????????????????????????????????????????????")
				print(HttpResponse(simplejson.dumps(mensajea),content_type='application/json'))
				return HttpResponse(simplejson.dumps(mensajea),content_type='application/json')
			except:
				mensaje = {"statusa":"False"}
				print("????????????????????????????????????????????????????????????????????")
...
##
				...
				print("????????????????????????????????????????????????????????????????????")
				print(HttpResponse(simplejson.dumps(mensaje),content_type='application/json'))
	...
	print(zk)
	bu = len(canti)
	ctx = {'canti':canti,'zk':zk,'bu':bu, 'suma':suma}
